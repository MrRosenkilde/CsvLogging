...
namespace CsvLogging
{
    public class CsvFunctions
    {
        ...
        private static IEnumerable<MemberInfo> MembersToLog(Type type, CollectionStrategy collectionStrategy) 
        {
            ...
            foreach (var memberInfo in objects) {
                if (memberInfo.TryGetAttribute(out LogByValueAttribute lgvAttr)
                    ...
                    if (memberInfo.TryGetCollectionType(out Type collectionType))
                        primitives.AddRange(
                            MembersToLog(collectionType, collectionStrategy)
                        );
                    else if (memberInfo.IsField())
                        primitives.AddRange(
                            MembersToLog(memberInfo.ToFieldType(), collectionStrategy)
                        );
                    else if (memberInfo.IsProperty())
                        primitives.AddRange(
                            MembersToLog(memberInfo.ToPropertyType(), collectionStrategy)
                        );
                    else throw new ArgumentException($"something went wrong with member: {memberInfo}");
                }
            }
            return primitives;
        }
        ...
        public static string ToLine(object baseObject, in CsvConfig config)
        {
            ...
            foreach (KeyValuePair<object, List<MemberInfo>> kvp in objectToLoggableMemberMap)
                foreach (var m in kvp.Value)
                    //if (!m.IsCollection())
                        sb.Append(
                            ToTerm(m.Name, kvp.Key, config)
                        );
                    //else {
                    //    sb.Append(
